...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:err="http://www.w3.org/2005/xqt-errors" exclude-result-prefixes="xs map diggs gml err">
    ...
    <xsl:template name="codeTypeValidation">
        ...
        <messageSet>
            ...
            <xsl:variable name="resourceUrl"
                select="'https://diggsml.org/def/validation/definedCodeTypes.xml'"/>

            <!-- Get the resource using the same pattern as dictionary validation -->
            <xsl:variable name="codeTypeResource"
                select="diggs:getResource($resourceUrl, base-uri(/))"/>
                    ...
                    <xsl:for-each select="$codeTypeResource//codeType">
                        ...
                        <xsl:try>
                            ...
                            <xsl:for-each select="$selectedElements">
                                ...
                                <xsl:variable name="codeSpaceDict" select="
                                        if (contains($codeSpaceValue, '#'))
                                        then
                                            substring-before($codeSpaceValue, '#')
                                        else $codeSpaceValue
                                "/>
                                ...
                                <xsl:choose>
                                    ...
                                    <xsl:when test="not(@codeSpace)">
                                        ...
                                        <xsl:sequence select="
                                                ...
                                                'ERROR',
                                                $elementPath,
                                                concat('&lt;', $elementName, '&gt; should reference a DIGGS Standard code list dictionary using the codeSpace attribute. Use one of these dictionaries to maximize interoperability:', $dictionaryList),
                                                $currentElement
                                                )"/>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        ...
                                        <xsl:choose>
                                            <xsl:when
                                                test="starts-with($codeSpaceValue, 'https://diggsml.org/def/codes/')">
                                            ...
                                            <xsl:otherwise>
                                                ...
                                                <xsl:sequence select="
                                                        ...
                                                        'WARNING',
                                                        $elementPath,
                                                        concat('&lt;', $elementName, '&gt; references a custom dictionary (&quot;', $codeSpaceDict, '&quot;) which may limit interoperability. If possible, use a DIGGS Standard code list dictionary:', $dictionaryList),
                                                        $currentElement
                                                        )"
                                                />
                                            </xsl:otherwise>
