...
root_attribs = {
    ...
    "{http://www.w3.org/2001/XMLSchema-instance}schemaLocation": "http://diggsml.org/schemas/2.6 http://diggsml.org/2.6/Diggs.xsd",
...
for _, row in definitions_df.iterrows():
    ...
    # (Add your code here if needed, based on your data structure)
...
# Directly add the processing instruction to the XML string

processing_instruction = '<?xml-stylesheet type="text/xsl" href="https://diggsml.org/def/stylesheets/codelists.xsl"?>\n'

final_xml_str = processing_instruction + pretty_xml_as_string
 
# Write the final XML string to the file, including the processing instruction

with open(xml_file_path, 'w', encoding='utf-8') as xml_file:

    xml_file.write(final_xml_str)
 
print(f"XML file with processing instruction created at: {xml_file_path}")
